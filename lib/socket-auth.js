...
function attachSocketAuthMiddleware(io) {
  io.use((socket, next) => {
    try {
      ...
      const accessToken = cookies[accessStore.ACCESS_COOKIE];
      const access = accessStore.getSession(accessToken);
      if (access?.account) {
        const pub = accessStore.publicAccount(access.account);
        socket.data.accessSession = {
