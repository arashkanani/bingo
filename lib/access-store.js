...
function createAccounts(plan, count = 1, note = '', email = '') {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const account = makeAccount(plan, note, normalizedEmail && n === 1 ? normalizedEmail : '');
    store.accounts.unshift(account);
...
function regenerateAccount(id) {
  ...
  const next = makeAccount(old.plan, old.note, old.email || '', {
