...
function getUsedDurationMs(account) {
  if (!account?.firstLoginAt) return null;
  const start = new Date(account.firstLoginAt).getTime();
  if (!Number.isFinite(start)) return null;
  let end = Date.now();
  if (account.expiresAt) {
    const expires = new Date(account.expiresAt).getTime();
    if (Number.isFinite(expires)) end = Math.min(end, expires);
  }
...
function publicAccount(account, { includePassword = true } = {}) {
  ...
  return {
    ...
    lastPlayerCount: Number(account.lastPlayerCount) || 0,
    peakPlayerCount: Number(account.peakPlayerCount) || 0,
    status: status.state,
    statusLabel: status.label,
    msRemaining: status.msRemaining,
    msUsed: getUsedDurationMs(account),
...
function recordPlayerCount(accountId, count) {
  const store = readStore();
  const account = store.accounts.find((a) => a.id === accountId);
  if (!account) return null;
  const n = Math.max(0, Math.floor(Number(count) || 0));
  account.lastPlayerCount = n;
  account.peakPlayerCount = Math.max(Number(account.peakPlayerCount) || 0, n);
  writeStore(store);
  return publicAccount(account);
}
